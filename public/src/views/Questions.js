...
export default {
  async mount(container) {
    ...
    const heading = el("div", { class: "mono stage-header__title question-title" }, "QUESTION 1/3");
    const timerDisplay = el("div", { class: "mono stage-header__timer" }, formatTimerLabel());
    ...
    const setTimerLabel = (value) => {
      timerDisplay.textContent = formatTimerLabel(value);
    };
    ...
    const stopTimer = (clearLabel = false) => {
      if (timerInterval) {
        clearInterval(timerInterval);
        timerInterval = null;
      }
      timerDeadline = null;
      if (clearLabel) setTimerLabel(null);
    };
    ...
    const roundSource = (rd && (Array.isArray(rd.hostItems) || Array.isArray(rd.guestItems)))
      ? rd
      : { hostItems: FALLBACK_ITEMS, guestItems: FALLBACK_ITEMS };

    const myItems = (myRole === "host" ? roundSource.hostItems : roundSource.guestItems) || [];
