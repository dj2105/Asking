...
export default {
  async mount(container) {
    ...
    container.innerHTML = "";
    const root = el("div", { class: "view view-marking view--full-center" });
    const stage = el("div", { class: "center-stage" });
    const stack = el("div", { class: "center-stage__stack" });

    const card = el("div", { class: "card" });
    const heading = el("h2", { class: "question-title" }, "MARKING");
    ...
    const showPostSubmit = (totalMs) => {
      ...
      const precise = formatSecondsPrecise(finalMs);
      showOverlay(`Waiting for ${oppName}`, `Round completed in ${precise}s`);
