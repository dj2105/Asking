...
export default {
  async mount(container) {
    ...
    const headerRow = el("div", { class: "stage-header" });
    const heading = el("div", { class: "mono stage-header__title marking-title" }, "MARKING");
    const timerDisplay = el(
      "div",
      { class: "mono stage-header__timer" },
      formatTimerLabel(MARKING_LIMIT_MS / 1000)
    );
    ...
    const rd = rdSnap.exists() ? (rdSnap.data() || {}) : null;
    const roundSource = (rd && (Array.isArray(rd.hostItems) || Array.isArray(rd.guestItems)))
      ? rd
      : { hostItems: PLACEHOLDER_ITEMS, guestItems: PLACEHOLDER_ITEMS };
    const oppItems = (oppRole === "host" ? roundSource.hostItems : roundSource.guestItems) || [];
    ...
    const setTimerLabel = (secs) => {
      timerDisplay.textContent = formatTimerLabel(secs);
    };
    ...
    stopRoomWatch = onSnapshot(rRef, (snap) => {
      ...
      if (ackMine && !published) {
        ...
        disableFns.forEach((fn) => { try { fn(); } catch {} });
        stopCountdown();
        setTimerLabel(lastTimedOut ? 0 : null);
