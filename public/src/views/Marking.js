...
export default {
  async mount(container) {
    ...
    const MARKING_LIMIT_SECONDS = MARKING_LIMIT_MS / 1000;

    const beginCountdown = (seconds = MARKING_LIMIT_SECONDS) => {
      if (published) return;
      const safeSeconds = Math.max(0, Math.ceil(Number(seconds) || MARKING_LIMIT_SECONDS));
      countdownDeadline = Date.now() + safeSeconds * 1000;
      countdownExpired = false;
      scoreStripStartTimer({ seconds: safeSeconds, onExpire: handleTimeout });
    };
    ...
    const handleTimeout = () => {
      ...
      scoreStripStopTimer({ keepVisible: true, reset: true });
    ...
    const submitMarks = async (timedOut = false) => {
      ...
      try {
        ...
        submitting = false;
        if (!published) {
          const fallbackMs = Math.max(remainingMs, 3_000);
          countdownExpired = false;
          beginCountdown(Math.ceil(fallbackMs / 1000));
          updateDoneState();
          hideOverlay();
        }
      }
    ...
    if (Array.isArray(existingMarks) && existingMarks.length === 3) {
      ...
      beginCountdown(MARKING_LIMIT_SECONDS);
      hideOverlay();
    }
