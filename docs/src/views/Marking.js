...
export default {
  async mount(container) {
    ...
    const resetFadeState = () => {
      ...
      [btnRight, btnUnknown, btnWrong].forEach((btn) => btn.classList.remove("is-fading"));
      content.classList.remove("is-fade-context");
    };

    const startSelectionFade = (sourceBtn) => {
      ...
      [btnRight, btnUnknown, btnWrong].forEach((btn) => {
        if (btn !== sourceBtn) btn.classList.add("is-fading");
      });
    };
    ...
    const showMark = (targetIdx, { animate = true, effect = "bump" } = {}) => {
    ...
    const scheduleAdvance = (currentIndex) => {
      ...
      advanceTimer = setTimeout(() => {
        ...
        if (next !== null && next !== undefined) {
          showMark(next, { animate: true, effect: "fade" });
        }
      }, 480);
    };
    ...
    stepButtons.forEach((btn, i) => {
      btn.addEventListener("click", () => {
        ...
        if (published || submitting) return;
        clearAdvanceTimer();
        showMark(i, { animate: true, effect: "bump" });
        reflectVerdicts();
        updateSubmitState();
      });
    });
