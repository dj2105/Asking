...
North Star
        ...
        •       Host prepares the room locally (packs + 3-char code) and only syncs the moments other screens need: countdowns, awards, final.
...
Firestore Contract (rooms/{CODE})
        ...
        •       markingReady.host.{round} / markingReady.guest.{round} — player clicked GO TO MARKING post-questions.
        •       markingCountdown.startAt / markingCountdown.round — optional short hold before flipping to marking once both ready.
...
3) Lobby (Jaime)
        •       Card layout with code input (3 chars). START throbs when valid.
...
5) Questions (local per role)
        •       Exactly 3 questions for the logged-in role (hostItems vs guestItems). Selecting an option auto-advances; no auto-submit.
        •       Submission shows SUBMIT (no interim clue). If opponent still answering, show a 5s maths clue under “WHICH YEAR...?” then GO TO MARKING; if both done, skip straight to GO TO MARKING.
        •       GO TO MARKING writes markingReady.{role}.{round}; when both ready, markingCountdown triggers the hop to marking.

6) Marking (local, 30s timer)
        •       Each player marks the opponent’s answers via ✓/✕ toggles. Timer enforced via marking.startAt.
        •       Submitting writes marking arrays + ack, then prompts for the WHICH YEAR...? maths guess in a slim 4-digit red field; SUBMIT appears only when valid.

7) Award (sync, 30s timer)
        •       Shows your answers with ✓/✕ result. Score strip recomputes from answers (marking is informational). Speed bonus shows as a gold +1 star beside round score; no Jemima box/banner.
        •       On timeout or Continue (both roles) → Countdown for next round; Round increments.

8) Maths (local)
        •       Two integer answers with units prompts. MathsPane stays mounted. Submits mathsAnswers + ack. Pre-maths award prompt uses WHICH YEAR...? with simplified input (no helper text).
...
Testing Checklist (Emulator)
        ...
        •       Jaime joins via Lobby with the chosen 3-char code → countdown armed (links.guestReady true) and both devices land on Countdown.
        •       Distinct question sets per role; answers auto-advance but require SUBMIT (no auto-submit).
        •       Post-submit flow: first finisher sees 5s WHICH YEAR...? clue + GO TO MARKING; second finisher sees GO TO MARKING directly. markingReady and markingCountdown drive the hop.
        •       Marking timer respects marking.startAt; ✓/✕ writes marking arrays + ack booleans; year guess only submits with valid 4-digit entry.
        •       Award updates scores via answer correctness only; speed bonus shows as gold +1 star; no Jemima clue box.
...
Style (non-negotiable)
        •       Courier everywhere; titles/answers bold. Maths guess input is narrow/tall with red numerals.
